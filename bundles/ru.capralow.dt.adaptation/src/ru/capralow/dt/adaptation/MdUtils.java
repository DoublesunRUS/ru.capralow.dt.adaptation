...
import ru.capralow.dt.adaptation.internal.AdaptationPlugin;
...
public final class MdUtils
{
    ...
    private static void addSubsystemsToListForObject(EList<Subsystem> subsystemsSource,
        EList<Subsystem> subsystemsDestination, MdObject mdObject)
    {
        for (Subsystem subsystem : subsystemsSource)
        {
            if (subsystem.getContent().contains(mdObject))
                subsystemsDestination.add(subsystem);
            addSubsystemsToListForObject(subsystem.getSubsystems(), subsystemsDestination, mdObject);
        }
    }
    ...
    public static MdObject getMdObject(String objectFullName, IV8Project v8Project)
    {
        IBmEmfIndexManager bmEmfIndexManager =
            AdaptationPlugin.getInstance().getInjector().getInstance(IBmEmfIndexManager.class);
    ...
    public static EList<Subsystem> getSubsystemsForObject(MdObject mdObject)
    {
        EList<Subsystem> subsystems = new BasicEList<>();

        IV8ProjectManager v8ProjectManager =
            AdaptationPlugin.getInstance().getInjector().getInstance(IV8ProjectManager.class);

        Configuration configuration = MdUtils.getConfigurationForProject(v8ProjectManager.getProject(mdObject));
        if (configuration == null)
            return subsystems;

        addSubsystemsToListForObject(configuration.getSubsystems(), subsystems, mdObject);
